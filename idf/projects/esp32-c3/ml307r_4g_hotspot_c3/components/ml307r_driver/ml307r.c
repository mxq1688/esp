...
static const char *TAG = "ML307R_C3";
...
esp_err_t ml307r_init(ml307r_t *ml307r)
{
    ...
    // ESP32-C3特定初始化
    ml307r->is_esp32c3 = true;
    ESP_LOGI(TAG, "Initializing ML307R for ESP32-C3 (RISC-V)");
    ...
    ESP_LOGI(TAG, "ML307R initialized successfully on ESP32-C3 GPIO%d(TX) GPIO%d(RX)", 
             ml307r->txd_pin, ml307r->rxd_pin);
    return ESP_OK;
}
...
esp_err_t ml307r_send_command(ml307r_t *ml307r, const char *command)
{
    ...
    ESP_LOGD(TAG, "C3 Sent: %s", command);
    return ESP_OK;
}
...
int ml307r_read_response(ml307r_t *ml307r, char *response, size_t max_len, uint32_t timeout_ms)
{
    ...
    if (len > 0) {
        response[len] = '\0';
        ESP_LOGD(TAG, "C3 Received: %s", response);
    }
...
ml307r_status_t ml307r_check_sim(ml307r_t *ml307r)
{
    ...
    ESP_LOGI(TAG, "C3: Checking SIM card status...");
    ...
    if (status == ML307R_OK) {
        ESP_LOGI(TAG, "C3: SIM card is ready");
    } else {
        ESP_LOGE(TAG, "C3: SIM card not ready or error");
    }
...
ml307r_status_t ml307r_check_network(ml307r_t *ml307r)
{
    ...
    ESP_LOGI(TAG, "C3: Checking network registration...");
    ...
    if (len > 0) {
        if (strstr(response, "+CREG: 0,1") != NULL || strstr(response, "+CREG: 0,5") != NULL) {
            ...
            ESP_LOGI(TAG, "C3: CS network registered (home)");
        } else if (strstr(response, "+CREG: 0,2") != NULL || strstr(response, "+CREG: 0,6") != NULL) {
            ml307r->net_status = ML307R_NET_REGISTERED_ROAMING;
            ESP_LOGI(TAG, "C3: CS network registered (roaming)");
        } else {
            ml307r->net_status = ML307R_NET_NOT_REGISTERED;
            ESP_LOGW(TAG, "C3: CS network not registered");
        }
    }
    ...
    if (len > 0) {
        if (strstr(response, "+CGREG: 0,1") != NULL || strstr(response, "+CGREG: 0,5") != NULL) {
            ESP_LOGI(TAG, "C3: PS network registered (home)");
            return ML307R_OK;
        } else if (strstr(response, "+CGREG: 0,2") != NULL || strstr(response, "+CGREG: 0,6") != NULL) {
            ESP_LOGI(TAG, "C3: PS network registered (roaming)");
            return ML307R_OK;
        } else {
            ESP_LOGW(TAG, "C3: PS network not registered");
...
ml307r_status_t ml307r_activate_pdp(ml307r_t *ml307r)
{
    ...
    ESP_LOGI(TAG, "C3: Activating PDP context...");
    ...
    if (status == ML307R_OK) {
        ESP_LOGI(TAG, "C3: PDP context activated successfully");
    } else {
        ESP_LOGE(TAG, "C3: Failed to activate PDP context");
    }
...
ml307r_status_t ml307r_get_ip_address(ml307r_t *ml307r)
{
    ...
    ESP_LOGI(TAG, "C3: Getting IP address...");
        ...
        if (ip_start != NULL) {
            ...
            if (ip_end != NULL) {
                ...
                if (ip_len < sizeof(ml307r->ip_address)) {
                    ...
                    ESP_LOGI(TAG, "C3: Got IP address: %s", ml307r->ip_address);
                    return ML307R_OK;
                }
            }
        }
    ...
    ESP_LOGE(TAG, "C3: Failed to get IP address");
    return ML307R_ERROR;
}
...
ml307r_status_t ml307r_connect_4g(ml307r_t *ml307r)
{
    ...
    ESP_LOGI(TAG, "C3: Starting 4G connection process...");
    ...
    if (ml307r_wait_response(ml307r, "OK", 3000) != ML307R_OK) {
        ESP_LOGE(TAG, "C3: AT test failed");
        return ML307R_ERROR;
    }
    ESP_LOGI(TAG, "C3: AT test passed");

    // 检查SIM卡
    if (ml307r_check_sim(ml307r) != ML307R_OK) {
        ESP_LOGE(TAG, "C3: SIM card check failed");
        return ML307R_ERROR;
    }

    // 检查网络注册
    if (ml307r_check_network(ml307r) != ML307R_OK) {
        ESP_LOGE(TAG, "C3: Network registration failed");
        return ML307R_ERROR;
    }

    // 激活PDP上下文
    if (ml307r_activate_pdp(ml307r) != ML307R_OK) {
        ESP_LOGE(TAG, "C3: PDP activation failed");
        return ML307R_ERROR;
    }
    ...
    ESP_LOGI(TAG, "C3: 4G connection established successfully!");
    return ML307R_OK;
}
